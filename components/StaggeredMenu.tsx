...
export const StaggeredMenu = ({
      ...
      if (socialLinks.length) {
        ...
        tl.to(
          captured,
          {
            ...
            ease: "power3.out",
            stagger: { each: 0.08, from: "start" },
            onComplete() { gsap.set(captured, { clearProps: "opacity" }); },
          },
          socialsStart + 0.04
        );
      }
  ...
  const prelayerColors = (() => {
    const raw = colors && colors.length ? colors.slice(0, 4) : ["#142020", "#1c2233"];
    const arr = [...raw];
    ...
    >
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <div className="sm-logo">
          {logoText ? (
            <Link href="/" className="sm-logo-text" onClick={closeMenu}>
              {logoText}
            </Link>
          ) : logoUrl ? (
